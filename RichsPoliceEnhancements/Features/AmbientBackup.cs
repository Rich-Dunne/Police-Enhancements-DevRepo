    ...
    class AmbientBackup
    {
        internal static void Main()
        {
            ...
            while (true)
            {
                ...
                {
                    foreach (Vehicle veh in Game.LocalPlayer.Character.GetNearbyVehicles(16).Where(v => v && v.IsPoliceVehicle && v != Game.LocalPlayer.Character.CurrentVehicle && v.HasDriver && v.Driver.IsAlive))
                    {
                        foreach (Ped ped in veh.Occupants.Where(p => p && p.IsAlive && !Functions.IsPedInPursuit(p)))
                        {
