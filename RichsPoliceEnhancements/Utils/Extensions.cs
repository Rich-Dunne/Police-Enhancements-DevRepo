...
namespace RichsPoliceEnhancements.Utils
{
	...
	internal static class Extensions
    {
        ...
        internal static bool IsAmbient(this Ped ped, PedType pedType = 0)
        {
            ...
            Game.LogTrivial($"[RPE Ambiance Check]: Nothing else has returned true by this point. (non-ambient)");
            Game.LogTrivial($"[RPE Ambiance Check]: Ped's active tasks are:");
            ...
            foreach (PedTask task in tasks)
            {
                ...
                {
                    Game.LogTrivial($"[RPE Ambiance Check]:		{task} ({(int)task})");
                }
            }
            return false;
        }

        private static bool IsAmbientInVehicle(Ped ped)
        {
            // In-vehicle controlled tasks
            var taskControlVehicle = Rage.Native.NativeFunction.Natives.GET_IS_TASK_ACTIVE<bool>(ped, 169); // From backup unit driving to player
